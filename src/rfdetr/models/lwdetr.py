...
class LWDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        if hs is not None:
            ...
            if self.pose_head is not None:
                # pose_head.forward(hs) returns a list of [B, Q, K, 3] per decoder layer
                outputs_kpts_list = self.pose_head(hs)  # list of [B, Q, K, 3]
                # Stack to [L, B, Q, K, 3] for aux-loss helper
                outputs_kpts = torch.stack(outputs_kpts_list, dim=0)  # [L, B, Q, K, 3]

            out = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord[-1]}
            if self.segmentation_head is not None:
                out["pred_masks"] = outputs_masks[-1]
            if self.pose_head is not None:
                out["pred_keypoints"] = outputs_kpts[-1]  # [B, Q, K, 3]
            if self.aux_loss:
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_masks, outputs_kpts=None):
        ...
        for i, (a, b) in enumerate(zip(outputs_class[:-1], outputs_coord[:-1])):
            d: dict = {"pred_logits": a, "pred_boxes": b}
            if outputs_masks is not None:
                d["pred_masks"] = outputs_masks[i]
            if outputs_kpts is not None:
                d["pred_keypoints"] = outputs_kpts[i]
            results.append(d)
        return results
...
def build_model(args: "BuilderArgs"):
    ...
    pose_head = (
        PoseHead(args.hidden_dim, num_keypoints)
        if pose_head_flag
        else None
    )
...
def build_criterion_and_postprocessors(args: "BuilderArgs"):
    ...
    pose_head_flag = getattr(args, "pose_head", False)
    if pose_head_flag:
        weight_dict["loss_oks"] = getattr(args, "oks_loss_coef", 5.0)
        weight_dict["loss_kpt_l1"] = getattr(args, "kpt_l1_loss_coef", 0.5)
        weight_dict["loss_kpt_vis"] = getattr(args, "kpt_vis_loss_coef", 2.0)
    # TODO this is a hack
    if args.aux_loss:
        aux_weight_dict = {}
